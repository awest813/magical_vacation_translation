...
if len(spirits) != len(types):
    raise ValueError("spirits and types must have the same number of entries")

count = len(spirits)
for ii in range(count):
    spirit0 = spirits[(ii - 1) % count]
    spirit1 = spirits[ii]
    spirit2 = spirits[(ii + 1) % count]
    type0 = types[(ii - 1) % count]
    type1 = types[ii]
    type2 = types[(ii + 1) % count]

    description = (template_string.replace("*S0*", spirit0)
        .replace("*S1*", spirit1)
        .replace("*S2*", spirit2)
        .replace("*T0*", type0)
        .replace("*T1*", type1)
